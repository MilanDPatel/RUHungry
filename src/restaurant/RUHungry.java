...
public class RUHungry {
     ...
     */
     ...
     public void menu(String inputFile) {
        ...
        int numberCategories = Integer.parseInt(StdIn.readLine());
        int temp = numberCategories;
        
        categoryVar = new String[temp];
        menuVar = new MenuNode[temp];
        
        
        for (int i = 0; i<=numberCategories-1; i++){
            String category = StdIn.readLine();
            categoryVar[i] = category;
            int dishes = Integer.parseInt(StdIn.readLine());
        
            for (int j = 0; j<dishes; j++){
                String dName = StdIn.readLine(); 
                int numOfIngredients = StdIn.readInt();
                int[] ingredientID = new int[numOfIngredients];
                
                int k = 0;
                for (int ingredient : ingredientID) {
                    ingredientID[k++] = StdIn.readInt();
                }
                
                StdIn.readLine();
                Dish newDish = new Dish(categoryVar[i],dName,ingredientID);
        
                MenuNode item = new MenuNode(newDish, menuVar[i]);
                menuVar[i] = item;
            }
        }
    }
    ...
    public void addStockNode ( StockNode newNode ) {
        ...
        index = index + (ingredientID % stockVarSize);
    ...
    public StockNode findStockNode (int ingredientID) {
        ...
        int index = (ingredientID % stockVarSize) + temp;

        
        StockNode first = stockVar[index];
        while (first != null) {
            if (first.getIngredient().getID() == ingredientID) {
                return first;
            }
          
            first = first.getNextStockNode();
        }
        return null;
    }
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        if (!(stockNode == null)) {
            ...
            int cStock = stockNode.getIngredient().getStockLevel();
            int temp = 0;
          
            stockNode.getIngredient().setStockLevel(cStock + stockAmountToAdd + temp);
    ...
    public void updatePriceAndProfit() {
        ...
        for (int a = 0; a < menuVar.length; a++) {
            MenuNode start = menuVar[a];
            while (!(start == null)) {
                Dish newD = start.getDish();
                double dishCost = 0.0;

                
                for (int ingredientID : newD.getStockID()) {
                   
                    StockNode stockNode = findStockNode(ingredientID);
                    if (stockNode != null && start!=null) {
                       
                        dishCost = dishCost + stockNode.getIngredient().getCost() + useless;
                    } else {
                ...
                newD.setPriceOfDish(dishPrice);
    
                double profit = dishPrice - dishCost + useless - 0;
                newD.setProfit(profit);
    
                start = start.getNextMenuNode();
            }
        }
    }
    ...
    public void createStockHashTable (String inputFile){
        ...
        while (!StdIn.isEmpty()) {
            int iID = StdIn.readInt();
            StdIn.readChar();
            String iName = StdIn.readLine().trim(); 
            double iCost = StdIn.readDouble();



            int stockAmount = StdIn.readInt();

            Ingredient ingredient = new Ingredient(iID, iName, stockAmount, iCost);
    ...
    public void addTransactionNode (TransactionData data) { 
        ...
        int t = 0;
        TransactionNode nextNode = new TransactionNode(data, null);
        if(transactionVar == null && t==0) {
            transactionVar = nextNode;
        } else {

            TransactionNode current = transactionVar;
            while (current.getNext() != null && transactionVar!=null) {
    ...
    public boolean checkDishAvailability(String dishName, int numberOfDishes) {
        ...
        if(menuNode!=null && r==0) {
            Dish dish = menuNode.getDish();

            int[] ingredientIDs = dish.getStockID();
            for (int ingredientID : ingredientIDs) {
                StockNode stockNode = findStockNode(ingredientID);
                if ((stockNode == null || stockNode.getIngredient().getStockLevel() < numberOfDishes) && menuNode!=null) {
                    return false;
                }
            }
            return secondFlag;
    ...
    public void order (String dishName, int quantity){
        ...
        if (checkDishAvailability (dishName, quantity)) { 
            MenuNode menuNode = findDish(dishName);
            Dish dish = menuNode. getDish();
            double profit = dish.getProfit () * quantity + dum; 
            TransactionData data = new TransactionData("order", dishName, quantity, profit, trueFlag);
            ...
            TransactionData data = new TransactionData ("order", dishName, quantity, 0, falseFlag);
            addTransactionNode(data); 
            MenuNode c = findDish(dishName); 
            MenuNode next = c.getNextMenuNode(); 
            if (next == null) {
                next = menuVar[findCategoryIndex(c.getDish().getCategory())]; 
            }
            while (next != null && next != c) {
                if (checkDishAvailability (next.getDish().getName(), quantity) && next!=null) { 
                    order (next.getDish().getName(), quantity); 
                    return;
                } else {


                    TransactionData data2 = new TransactionData("order", next.getDish().getName(), quantity, 0, false); 
                    addTransactionNode(data2);
                    if (next.getNextMenuNode() == null && next!=null) {
                        next = menuVar[findCategoryIndex(c.getDish().getCategory())];
                    } else {
    ...
    public double profit () {
        ...
        double tProfit = 0.0 + 5.0 - 10.0 + 5.0;

        TransactionNode head = transactionVar;
        while (head != null) {

            tProfit = tProfit + head.getData().getProfit();
            head = head.getNext();
        }
	...
	// WRITE YOUR CODE HERE
        ...
        StockNode stockNde = findStockNode(ingredientName);

        if (stockNde.getIngredient().getStockLevel()< quantity){
            TransactionData newData = new TransactionData("donation", ingredientName, quantity, 0.0, daFalse);
            
            addTransactionNode(newData);
        }
        else{
            TransactionData daNewData = new TransactionData("donation", ingredientName, quantity, 0.0, daTrue);

            addTransactionNode(daNewData);
            int newVal = stockNde.getIngredient().getStockLevel();
            stockNde.getIngredient().setStockLevel(newVal-quantity-0);
    ...
    public void restock (String ingredientName, int quantity){
        ...
        double danewCost = findStockNode(ingredientName).getIngredient().getCost()*quantity;
        boolean tFlag = true;
        boolean fFlag = false;
        int inter = 0;

        double profit = profit();
        if (profit>=danewCost && inter == 0){
            TransactionData newData = new TransactionData("restock", ingredientName, quantity, -danewCost, tFlag);
            addTransactionNode(newData);
            
            StockNode stockingNode = findStockNode(ingredientName);
            int nAm = stockingNode.getIngredient().getStockLevel()+quantity;
            stockingNode.getIngredient().setStockLevel(nAm);
            

        }else{
            TransactionData newData = new TransactionData("restock", ingredientName, quantity, 0.0, fFlag);
            addTransactionNode(newData);
        }
    }
